...
class FilesApi(ApiShared):
    ...
    def create_file(
        ...
        if not NyFile.is_file_type(type_name):
            raise ValidationError(
                f"type {type_name!r} is not a file type — use /api/files only for File/Document/Image"
            )
        if not filename.strip():
            raise ValidationError("filename must not be empty")
        if len(data) > NyFile.MAX_UPLOAD_BYTES:
            raise ValidationError(
                f"file exceeds the {NyFile.MAX_UPLOAD_BYTES // (1024 * 1024)} MiB upload cap"
            )
        if not NyFile.accepts_mime(type_name, mime):
            raise ValidationError(
