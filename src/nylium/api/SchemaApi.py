...
if TYPE_CHECKING:
    ...
    from nylium.api.FunctionsApi import FunctionsApi as _SchemaBase
else:
...
class SchemaApi(_SchemaBase):
    ...
    def reorder_props(cls, type_name: str, keys: list[str]) -> Type:
        ...
        existing = [prop.key for prop in NyProp.all_for(owner)]
    ...
    def sync_props(
        ...
        self_arrays = {
            ...
            if NyType.is_array_name(value_type_name)
            and NyType.element_name(value_type_name) == type_name
        }
        if renames:
            items = [
                (
        ...
        NyProp.sync_schema(owner, resolved)
