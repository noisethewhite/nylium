...
class FunctionsApi(_FunctionsBase):
    ...
    def _member_props(cls, element_type_name: str) -> list[tuple[str, str]] | None:
        ...
        element = NyType.by_name(element_type_name)
        if element is None:
            return None
        # effective schema (ADR-0013); spec names keep Any<Trait> members
        # inspectable for formula validation
        return [
            (prop.key, prop.value_spec_name()) for prop in NyProp.effective_for(element)
        ]
    ...
    def _rewrite_dependent_formulas(
        ...
        array_type_row = next(types.where(name=NyType.array_name(type_name)), None)
    ...
    def _check_formula_prop(
        ...
        if formula is None:
            return
        if value_type_name == NyInteger.TYPE_NAME:
            if not Formula.is_bare_count(formula):
                raise ValidationError(
                    "an Integer formula prop must be a bare COUNT(<array>) call"
                )
        elif value_type_name != NyNumeric.TYPE_NAME and NyType.unit_param_of(value_type_name) is None:
            raise ValidationError(
                f"a formula prop must be {NyNumeric.TYPE_NAME} (or {NyInteger.TYPE_NAME} for a bare COUNT, or Numeric<Unit>), got {value_type_name!r}"
            )
    ...
    def _is_ordered_scalar(cls, spec_name: str) -> bool:
        ...
        return (
            ...
            in {
                ...
                NyNumeric.TYPE_NAME,
                NyDate.TYPE_NAME,
                NyDatetime.TYPE_NAME,
            }
            or NyType.unit_param_of(spec_name) is not None
        )
    ...
    def _is_string_like(cls, spec_name: str) -> bool:
        ...
        if spec_name == NyString.TYPE_NAME:
            return True
        target = NyType.by_name(spec_name)
    ...
    def _check_collect_prop(
        ...
        if collect is None:
            return
        if not NyType.is_array_name(value_type_name):
            raise ValidationError(
                f"a collect prop must be an array (Array<T>), got {value_type_name!r}"
            )
        element_name = NyType.element_name(value_type_name)
        element = NyType.by_name(element_name)
        if element is None:
