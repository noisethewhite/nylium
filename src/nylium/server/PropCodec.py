...
class PropCodec:
    ...
    def _decode_value(cls, value: PropValue, type_name: str) -> PropInput:
        ...
        if NyType.unit_param_of(type_name) is not None:
            ...
            # part membership and conversion happen in NyUnit.validate,
        ...
        if NyType.is_array_name(type_name):
            ...
            element_type = NyType.by_name(element_name)
    ...
    def _coerce_scalar(cls, raw: ScalarPayload | None, type_name: str) -> ScalarPayload | None:
        ...
        if scalar is NyString and type(raw) is str:
            return raw
        if scalar is NyInteger and type(raw) is int:
            return raw
        if scalar is NyBoolean and type(raw) is bool:
            return raw
        if scalar is NyNumeric and type(raw) in (int, float, Decimal):
            # str() detour: Decimal(float) would bake in binary float noise
            return Decimal(str(raw))
        if scalar is NyNumeric and type(raw) is str:
            try:
                return Decimal(raw)
            except InvalidOperation:
                raise TypeError(f"value {raw!r} is not a valid {type_name}") from None
        if scalar is NyDatetime:
