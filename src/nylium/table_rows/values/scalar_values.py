...
tables (ADR-0031, ADR-0033): one shared store base, nine thin subclasses.

Composite PK ``(inst_uuid, prop_uuid)``. Writes go through explicit
insert/update. Each ``WScalar`` marker binds ``SCALAR`` to its store
subclass; each store subclass binds ``__row__`` to its mapped Row (the Row
IS the mapped dataclass — there is no ``TABLE_*``).

This module imports no concrete Row subclasses — each scalar module imports
``ScalarValuesTable`` from here, so importing the Row types here would
