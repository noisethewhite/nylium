...
def _is_numeric(type_name: str) -> bool:
    ...
    if type_name == NyInteger.TYPE_NAME:
        return True
    if type_name == NyNumeric.TYPE_NAME:
        return True
    return type_name.startswith(NyNumeric.TYPE_NAME + "<") and type_name.endswith(">")
...
class Formula:
    ...
    def validate(
        ...
        for call in _calls(ast):
            ...
            if not NyType.is_array_name(value_type):
                raise ValidationError(
            ...
            element_name = NyType.element_name(value_type)
