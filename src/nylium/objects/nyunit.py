"""NyUnit: persistence of a `Numeric<Unit>` prop — one numeric_values row
storing the canonical (base-part) magnitude in `value` and the part name
as entered in `unit`.

A bare `Numeric` prop never reaches this facade: `NyType.unit_param_of`
is None for it and NyObject dispatches to the plain scalar path. The
