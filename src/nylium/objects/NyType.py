"""NyType: handle on a row of the `types` table.
...
Prop queries live on NyProp, not here: nyprop imports nytype (for
value_type), so nytype must not import nyprop back — that keeps the
...
class NyType:
    ...
    def unit_param_of(cls, type_name: str) -> str | None:
        ...
        Whether the parameter actually names a unit type is NyUnit's
    ...
    def by_name(cls, name: str) -> "NyType | None":
