"""Array values store — element rows, the array-tag reverse projection
(ADR-0005) and the collect-range lookup (ADR-0025).

The statements moved here from ``objects/warray_values.py`` (ADR-0030
phase C) and now live on the ``ArrayValues`` store, so the objects layer
stops owning SQL (ADR-0031). ``TABLE_ArrayValues`` stays in
``tables/values/array_values.py``.
"""
...
class ArrayValues(Table[tuple[UUID, int], ArrayValue]):
    ...
    @Database.use_same_session
    def array_tag_rows(
        cls, element_uuid: UUID, array_type_name: str, name_prop_key: str
    ...
    @Database.use_same_session
    def collect_range(
        cls, prop_uuid: UUID, spec_name: str, lo: object, hi: object
    ) -> list[UUID]:
