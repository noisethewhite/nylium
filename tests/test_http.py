...
def _function_draft(input_type, output_type, name, nodes, edges):
    """Wire shape of a Create/Update function draft (ADR-0029: no input link)."""
    return {
...
def test_function_lifecycle_over_http(auth_client: TestClient) -> None:
    ...
    dsl.create_type(
        auth_client,
        "Invoice",
        {"name": "String", "total": "Numeric", "tax": "Numeric", "taxed": "Numeric"},
    )
    ...
    # bind to inv.taxed and read the computed value (total = 100)
    bound = auth_client.put(
        f"/api/objects/{inv['uuid']}/props/taxed/function",
        json={"function_uuid": fn_uuid},
    )
    assert bound.status_code == 200, bound.text
    assert bound.json()["props"]["taxed"]["value"] == "100"
    ...
    reloaded = auth_client.get(f"/api/objects/{inv['uuid']}").json()
    assert reloaded["props"]["taxed"]["value"] == "500"
...
def test_function_validation_over_http(auth_client: TestClient) -> None:
    ...
    dsl.create_type(
        auth_client,
        "Invoice",
        {"name": "String", "total": "Numeric", "taxed": "Numeric"},
    )
    inv = dsl.create_object(
        auth_client, "Invoice", {"name": {"value": "i1"}, "total": {"value": "100"}}
    )
    ...
    bound = auth_client.put(
        f"/api/objects/{inv['uuid']}/props/taxed/function",
        json={"function_uuid": fn["uuid"]},
    )
    assert bound.status_code == 200, bound.text
    guarded = auth_client.patch(
        f"/api/objects/{inv['uuid']}",
        json={"props": {"taxed": {"value": "1"}}},
    )
